...
struct queue {
	...
	struct msgpack queue[QUEUESIZE];
};
...
lclear(lua_State *L) {
	...
	for (i=q->head;i<q->tail;i++) {
		struct msgpack *np = &q->queue[i % q->cap];
...
expand_queue(lua_State *L, struct queue *q) {
	struct queue *nq = lua_newuserdata(L, sizeof(struct queue) + q->cap * sizeof(struct msgpack));
...
push_data(lua_State *L, int fd, void *buffer, int size, int clone) {
	...
	struct msgpack *np = &q->queue[q->tail];
	if (++q->tail >= q->cap)
		q->tail -= q->cap;
	np->id = fd;
	np->buffer = buffer;
...
#if SIZE_LENGTH == 4
	int r = (int)buffer[3] << 24 | (int)buffer[2] << 16 | (int)buffer[1] << 8 | (int)buffer[0];
#else
...
lpop(lua_State *L) {
	...
	if (q == NULL || q->head == q->tail)
		return 0;
	struct msgpack *np = &q->queue[q->head];
	if (++q->head >= q->cap) {
		q->head = 0;
	}
...
lpack(lua_State *L) {
	...
	uint8_t * buffer = skynet_malloc(len + SIZE_LENGTH);
	write_size(buffer, len);
	memcpy(buffer+SIZE_LENGTH, ptr, len);

	lua_pushlightuserdata(L, buffer);
	lua_pushinteger(L, len + SIZE_LENGTH);
...
ltostring(lua_State *L) {
	...
	if (ptr == NULL || size < 4) {
		...
		lua_pushlstring(L, (const char *)ptr + 4, size - 4);
		int code = read_size((uint8_t *) ptr);
